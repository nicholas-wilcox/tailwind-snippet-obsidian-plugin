...
export class SettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		const enablePreflightSetting = new Setting(containerEl)
			...
			.setDesc(
				`Adds TailwindCSS's Preflight to the generated stylesheet.
					 (NOTE: Not all Preflight styles will be applied, and Obsidian's UI may be affected.)`,
			)
			.addToggle((toggle) =>
				toggle
		...
		const addPrefixSelectorSetting = new Setting(containerEl)
			...
			.setDesc(
				`Prefixes all selectors in Tailwind's style rules with another selector.
		...
		const prefixSelectorSetting = new Setting(containerEl)
			...
			.setDesc(
				`Will be combined with all Tailwind selectors using a descendant combinator.
		...
		const themeConfig = new Setting(containerEl)
			...
			.setDesc(
				`The theme of the Tailwind configuration object.
		...
		const contentConfig = new Setting(containerEl)
			...
			.setDesc(
				`A comma-separated list of file globs that will be inspected by Tailwind
